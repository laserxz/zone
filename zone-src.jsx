    ...
    const APPS = [
      { id: "showco", name: "ShowCo", tagline: "Laser show management \u2014 cues, crew, gear & runsheets", url: "https://showco.zone.net.au", icon: "\uD83C\uDF9B\uFE0F", color: "#e040fb", gradient: "linear-gradient(135deg, #e040fb 0%, #7c4dff 100%)", category: "Laser Show Tools", crossCategory: ["Finance"] },
      { id: "lasermpe", name: "MPE Calculator", tagline: "Maximum Permissible Exposure safety calculator", url: "https://lasermpe.zone.net.au", icon: "\u26A0", color: "#00ff88", gradient: "linear-gradient(135deg, #00ff88 0%, #00b4d8 100%)", category: "Laser Show Tools" },
      { id: "lasermaze", name: "LaserMaze", tagline: "Interactive laser maze challenge game", url: "https://lasermaze.zone.net.au", icon: "\u2B21", color: "#ff003c", gradient: "linear-gradient(135deg, #ff003c 0%, #ff6b35 100%)", category: "Laser Show Tools" },
      { id: "swms", name: "SWMS", tagline: "Safe Work Method Statements made simple", url: "https://swms.zone.net.au", icon: "\u2713", color: "#ffd60a", gradient: "linear-gradient(135deg, #ffd60a 0%, #ff9f1c 100%)", category: "Laser Show Tools" },
      { id: "conductor", name: "Conductor", tagline: "Hand tracking + live MIDI control for laser shows", url: "https://conductor.zone.net.au", icon: "\uD83E\uDD4C", color: "#7c4dff", gradient: "linear-gradient(135deg, #7c4dff 0%, #448aff 100%)", category: "Laser Show Tools" },
      { id: "laserface", name: "LaserFace", tagline: "Face tracking \u2192 OSC for laser software", url: "https://laserface.zone.net.au", icon: "\uD83C\uDFAD", color: "#ff375f", gradient: "linear-gradient(135deg, #ff375f 0%, #bf5af2 100%)", category: "Laser Show Tools" },
      { id: "bpm", name: "BPM Detector", tagline: "Detect tempo from audio files with waveform analysis", url: "https://bpm.zone.net.au", icon: "\u266B", color: "#bf5af2", gradient: "linear-gradient(135deg, #bf5af2 0%, #ff375f 100%)", category: "Laser Show Tools" },
      ...
      { id: "mortgage", name: "Mortgage Calculator", tagline: "Australian mortgage repayment & comparison tool", url: "https://mortgagecalc.zone.net.au", icon: "\u2302", color: "#2ec4b6", gradient: "linear-gradient(135deg, #2ec4b6 0%, #00ff88 100%)", category: "Utilities", crossCategory: ["Finance"] },
      ...
      { id: "popupcards", name: "PopUp Cards", tagline: "Design laser-cut pop-up cards in browser", url: "https://popupcards.zone.net.au", icon: "\u2702", color: "#ff6b9d", gradient: "linear-gradient(135deg, #ff6b9d 0%, #c44569 100%)", category: "Entertainment" },
    ];
      ...
      return (
        <a href={app.url} target="_blank" rel="noopener noreferrer" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ display: "block", textDecoration: "none", color: "inherit", animation: `cardFadeIn 0.4s ease ${index * 0.05}s both` }}>
          <div style={{ position: "relative", borderRadius: 12, overflow: "hidden", background: "rgba(10,14,20,0.85)", border: `1px solid ${hovered ? app.color + "88" : "rgba(255,255,255,0.06)"}`, backdropFilter: "blur(20px)", transition: "all 0.3s cubic-bezier(0.22, 1, 0.36, 1)", transform: hovered ? "translateY(-4px) scale(1.02)" : "translateY(0) scale(1)", boxShadow: hovered ? `0 12px 40px ${app.color}22, 0 0 30px ${app.color}11, inset 0 1px 0 rgba(255,255,255,0.05)` : "0 2px 16px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.03)", cursor: "pointer" }}>
            <div style={{ height: 60, background: app.gradient, position: "relative", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
              ...
              <span style={{ fontSize: 32, filter: "drop-shadow(0 4px 12px rgba(0,0,0,0.4))", position: "relative", zIndex: 2, transition: "transform 0.3s ease", transform: hovered ? "scale(1.15) rotate(-5deg)" : "scale(1)" }}>{app.icon}</span>
              <div style={{ position: "absolute", top: 6, right: 6, background: "rgba(0,0,0,0.55)", backdropFilter: "blur(10px)", padding: "2px 7px", borderRadius: 20, fontSize: 9, fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.05em", color: "rgba(255,255,255,0.7)", textTransform: "uppercase" }}>{app.category}</div>
            </div>
            <div style={{ padding: "10px 12px 12px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 3 }}>
                <h3 style={{ margin: 0, fontSize: 14, fontFamily: "'Orbitron', sans-serif", fontWeight: 700, color: "#fff", letterSpacing: "0.02em" }}>{app.name}</h3>
              ...
              <p style={{ margin: 0, fontSize: 11, lineHeight: 1.4, color: "rgba(255,255,255,0.5)", fontFamily: "'IBM Plex Sans', sans-serif" }}>{app.tagline}</p>
              <div style={{ marginTop: 6, padding: "3px 7px", background: "rgba(255,255,255,0.03)", borderRadius: 6, border: "1px solid rgba(255,255,255,0.04)", fontSize: 9, fontFamily: "'JetBrains Mono', monospace", color: app.color, opacity: 0.7, letterSpacing: "0.03em" }}>{app.url.replace("https://", "")}</div>
            </div>
          </div>
        </a>
      );
      ...
      return (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, justifyContent: "center", marginBottom: 24 }}>
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button key={cat} onClick={() => onChange(cat)} style={{ padding: "8px 18px", borderRadius: 30, border: active === cat ? `1px solid ${accent}` : "1px solid rgba(255,255,255,0.1)", background: active === cat ? `${accent}1f` : "rgba(255,255,255,0.03)", color: active === cat ? accent : "rgba(255,255,255,0.5)", fontFamily: "'JetBrains Mono', monospace", fontSize: 12, letterSpacing: "0.06em", textTransform: "uppercase", cursor: "pointer", transition: "all 0.25s ease" }}>{cat}</button>
            );
          })}
        </div>
      );
    ...
    function TabbedPanel({ feedbackText, setFeedbackText, feedbackApp, setFeedbackApp, feedbackSent, handleFeedbackSubmit }) {
      const [tab, setTab] = useState("about");
      const tabs = ["about", "feedback", "donate"];
      return (
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 24px 60px" }}>
          <div style={{ display: "flex", gap: 0, justifyContent: "center", marginBottom: 20, borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
            {tabs.map((t) => (
              <button key={t} onClick={() => setTab(t)} style={{ padding: "10px 28px", background: "none", border: "none", borderBottom: tab === t ? "2px solid #00ff88" : "2px solid transparent", color: tab === t ? "#00ff88" : "rgba(255,255,255,0.4)", fontFamily: "'Orbitron', sans-serif", fontSize: 13, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer", transition: "all 0.25s ease" }}>{t}</button>
            ))}
          </div>
          ...
          {tab === "feedback" && (
            <div style={{ maxWidth: 700, margin: "0 auto", padding: "0", borderRadius: 16, background: "rgba(10,14,20,0.7)", border: "1px solid rgba(255,255,255,0.06)", backdropFilter: "blur(20px)", overflow: "hidden" }}>
              <div className="feedback-paths" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, padding: "24px 24px 0" }}>
    ...
    function ZoneLanding() {
      ...
      const filtered = category === "All"
        ? APPS
        : APPS.filter((a) => a.category === category || (a.crossCategory && a.crossCategory.includes(category)));
      ...
      return (
        <>
          ...
          {/* Navigation with inline tagline */}
          <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "0 24px", transition: "all 0.3s ease", background: scrollY > 50 ? "rgba(6,10,16,0.9)" : "rgba(6,10,16,0.7)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
            <div className="nav-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 52, gap: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                <span style={{ display: "inline-block", width: 8, height: 8, borderRadius: "50%", background: "#00ff88", boxShadow: "0 0 12px #00ff88", animation: "pulse 2s ease infinite", flexShrink: 0 }} />
                <span style={{ fontFamily: "'Orbitron', sans-serif", fontWeight: 800, fontSize: 16, color: "#00ff88", letterSpacing: "0.06em", whiteSpace: "nowrap" }}>ZONE.NET.AU</span>
                <span style={{ fontFamily: "'IBM Plex Sans', sans-serif", fontSize: 12, color: "rgba(255,255,255,0.35)", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ color: "rgba(255,255,255,0.15)" }}>&mdash;</span>
                  Free web apps, built in Australia
                </span>
              </div>
              <div style={{ display: "flex", gap: 4, flexWrap: "wrap", flexShrink: 0 }}>
                {["apps", "about", "feedback", "donate"].map((id) => (
                  <button key={id} onClick={() => { if (id === "about" || id === "feedback" || id === "donate") { document.getElementById("info-panel")?.scrollIntoView({ behavior: "smooth" }); } else { document.getElementById(id)?.scrollIntoView({ behavior: "smooth" }); }}} className="nav-link" style={{ background: "none", cursor: "pointer" }}>{id}</button>
                ))}
              </div>
            </div>
          </nav>
          ...
          <div style={{ position: "relative", zIndex: 2 }}>
            {/* Apps Section */}
            <section id="apps" style={{ maxWidth: 1200, margin: "0 auto", padding: "72px 24px 40px" }}>
              <CategoryFilter active={category} onChange={setCategory} />
              <div className="app-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(175px, 1fr))", gap: 16 }}>
            ...
            <section id="info-panel">
              <TabbedPanel feedbackText={feedbackText} setFeedbackText={setFeedbackText} feedbackApp={feedbackApp} setFeedbackApp={setFeedbackApp} feedbackSent={feedbackSent} handleFeedbackSubmit={handleFeedbackSubmit} />
            </section>
